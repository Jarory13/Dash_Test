...
public class AStar {
	...
	public static bool FindPath() {
		...
		if (!gridMap[startIndex.x][startIndex.y] || !gridMap[endIndex.x][endIndex.y]) {
	...
	private static void CheckAdjacents() {
		...
		nodeIndex.x = currentNode.nodeIndex.x - 1; // Up Row Adjacent Node
		nodeIndex.y = currentNode.nodeIndex.y;		// Current Column Adjacent Node
		nodeIndexes.Add(nodeIndex);
		
		nodeIndex = new NodeIndex();
		nodeIndex.x = currentNode.nodeIndex.x - 1; // Up Row Adjacent Node
		nodeIndex.y = currentNode.nodeIndex.y + 1;	// Right Column Adjacent Node
		nodeIndexes.Add(nodeIndex);
		
		nodeIndex = new NodeIndex();
		nodeIndex.x = currentNode.nodeIndex.x; 	// Current Row Adjacent Node
		nodeIndex.y = currentNode.nodeIndex.y + 1;	// Right Column Adjacent Node
		nodeIndexes.Add(nodeIndex);
		
		nodeIndex = new NodeIndex();
		nodeIndex.x = currentNode.nodeIndex.x + 1; // Down Row Adjacent Node
		nodeIndex.y = currentNode.nodeIndex.y + 1;	// Right Column Adjacent Node
		nodeIndexes.Add(nodeIndex);
		...
		nodeIndex = new NodeIndex();
		nodeIndex.x = currentNode.nodeIndex.x + 1; // Down Row Adjacent Node
		nodeIndex.y = currentNode.nodeIndex.y - 1;	// Left Column Adjacent Node
		nodeIndexes.Add(nodeIndex);
		...
		foreach (NodeIndex n in nodeIndexes) {
			if (
				n.x < 0 || n.y < 0 || 
				n.x >= gridMap.Count || n.y >= gridMap[0].Count ||
				false == gridMap[n.x][n.y] ||
		...
		foreach (NodeIndex indx in nodeIndexes) {
			...
			if (null == node) {
				...
				node = new Node();
				node.nodeIndex.x = indx.x;
				node.nodeIndex.y = indx.y;
				openList.Add(node);
				
				if (
					node.nodeIndex.x != currentNode.nodeIndex.x && 
					node.nodeIndex.y != currentNode.nodeIndex.y
				) {
	...
	*/
	...
	public static int HeuristicChebyshev(Node n) {
		return 10 * Mathf.Max(
			Mathf.Abs(endIndex.x - n.nodeIndex.x), 
			Mathf.Abs(endIndex.y - n.nodeIndex.y)
		);
	}
	
	public static int HeuristicEuclidean(Node n) {
		return 10 * (int)Mathf.Sqrt(
			Mathf.Pow(endIndex.x - n.nodeIndex.x, 2) + 
			Mathf.Pow(endIndex.y - n.nodeIndex.y, 2)
		);
	}
	...
	public static int HeuristicBalanceGH(Node n) {
		int xDistance = Mathf.Abs(endIndex.x - n.nodeIndex.x);
		int yDistance = Mathf.Abs(endIndex.y - n.nodeIndex.y);
	...
	public static bool ContainsNodeIndex(List<Node> list, NodeIndex indx) {
		...
		foreach (Node n in list) {
			if (n.nodeIndex.x == indx.x && n.nodeIndex.y == indx.y) {
				return true;
			}
		}

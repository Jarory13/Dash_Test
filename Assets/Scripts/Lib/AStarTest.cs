...
public class AStarTest : MonoBehaviour {
	...
	{
				Debug.Log ("HandleAStarOnPathFinded... path: " + path.Count);

				int pathCount = path.Count;
				for (int i = 0; i < pathCount; i++) {
						Debug.Log ("Path node... : " + path [i].nodeIndex.x + ", " + path [i].nodeIndex.y);
				}
		...
		for (int i = 0; i < AStar.gridMap.Count; i++) {
			for (int j = 0; j < AStar.gridMap[i].Count; j++) {
				...
				if (false == AStar.gridMap[i][j]) {
					...
					AStar.startIndex.x == i && AStar.startIndex.y == j ||
					AStar.endIndex.x == i && AStar.endIndex.y == j
				) {
